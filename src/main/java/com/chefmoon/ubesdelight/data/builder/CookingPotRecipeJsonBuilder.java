...
public class CookingPotRecipeJsonBuilder implements CraftingRecipeJsonBuilder {
    ...
    private CookingPotRecipeJsonBuilder(CookingPotRecipeCategory cookingCategory, ItemConvertible output, int outputCount, ItemStack container, List<Ingredient> inputs, float experience, int cookingTime) {
    ...
    public static CookingPotRecipeJsonBuilder create(CookingPotRecipeCategory cookingCategory, ItemConvertible output, int outputCount, ItemStack container, List<Ingredient> inputs, float experience, int cookingTime) {
        return new CookingPotRecipeJsonBuilder(cookingCategory, output, outputCount, container, inputs, experience, cookingTime);
    }
    ...
    public static CookingPotRecipeJsonBuilder create(CookingPotRecipeCategory cookingCategory, ItemConvertible output, int outputCount, int cookingTime, List<Ingredient> inputs) {
        return new CookingPotRecipeJsonBuilder(cookingCategory, output, outputCount, ItemStack.EMPTY, inputs, .0F, cookingTime);
    }
    ...
    public static CookingPotRecipeJsonBuilder create(CookingPotRecipeCategory cookingCategory, ItemConvertible output, int outputCount, float experience, int cookingTime, List<Ingredient> inputs) {
        return new CookingPotRecipeJsonBuilder(cookingCategory, output, outputCount, ItemStack.EMPTY, inputs, experience, cookingTime);
    }
    ...
    public static CookingPotRecipeJsonBuilder create(CookingPotRecipeCategory cookingCategory, ItemConvertible output, float experience, int cookingTime, List<Ingredient> inputs) {
        return new CookingPotRecipeJsonBuilder(cookingCategory, output, 1, ItemStack.EMPTY, inputs, experience, cookingTime);
    }
    ...
    public void offerTo(Consumer<RecipeJsonProvider> exporter, Identifier recipeId) {
        ...
        exporter.accept(new CookingPotRecipeJsonProvider(new Identifier(recipeId.getNamespace(),fdCookingPrefix + recipeId.getPath()), this.group == null ? "" : this.group, this.cookingCategory, this.inputs, this.output, this.outputCount, this.container, this.experience, this.cookingTime, this.advancementBuilder, new Identifier(recipeId.getNamespace(),"recipes/" + fdCookingPrefix + recipeId.getPath())));
    ...
    public static class CookingPotRecipeJsonProvider implements RecipeJsonProvider {
        ...
        public void serialize(JsonObject json) {
            ...
            resultObject.addProperty("item", Registry.ITEM.getId(this.result).toString());
            ...
            if (!this.container.isEmpty() && this.result.getRecipeRemainder() != null && !this.container.isOf(this.result.getRecipeRemainder())) {
                ...
                containerObject.addProperty("item", Registry.ITEM.getId(this.container.getItem()).toString());

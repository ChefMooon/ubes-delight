...
public class VillagerEvents {
    ...
    public static void onVillagerTrades(VillagerTradesEvent event) {
        ...
        if (professionKey.getPath().equals("farmer")) {
            trades.get(1).add(emeraldForItemsTrade(UbesDelightItems.UBE.get(),26, 16, 2));
            trades.get(1).add(emeraldForItemsTrade(UbesDelightItems.GARLIC.get(),26, 16, 2));
            trades.get(1).add(emeraldForItemsTrade(UbesDelightItems.GINGER.get(),26, 16, 2));
            trades.get(1).add(emeraldForItemsTrade(UbesDelightItems.LEMONGRASS.get(),20, 16, 5));// 2xp? same as wheat? others same as potato
        }
    }

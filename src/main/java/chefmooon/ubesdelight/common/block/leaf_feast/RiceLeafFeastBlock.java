...
public class RiceLeafFeastBlock extends SimpleLeafFeastBlock {
    ...
    public ItemStack getCloneItemStack(LevelReader level, BlockPos pos, BlockState state) {
        if (state.getValue(SERVINGS) == 3) {
            if (servingItem.get() == BuiltInRegistries.ITEM.get(ResourceLocation.fromNamespaceAndPath("farmersdelight", "cooked_rice"))) {
                return new ItemStack(UbesDelightItems.LEAF_FEAST_COOKED_RICE_HALF.get());
            } else if (servingItem.get() == BuiltInRegistries.ITEM.get(ResourceLocation.fromNamespaceAndPath("farmersdelight", "fried_rice"))) {
                return new ItemStack(UbesDelightItems.LEAF_FEAST_FRIED_RICE_HALF.get());
            } else if (servingItem.get() == UbesDelightItems.SINANGAG.get()) {
                return new ItemStack(UbesDelightItems.LEAF_FEAST_SINANGAG_HALF.get());
            }
        } else {
            if (servingItem.get() == BuiltInRegistries.ITEM.get(ResourceLocation.fromNamespaceAndPath("farmersdelight", "cooked_rice"))) {
                return new ItemStack(UbesDelightItems.LEAF_FEAST_COOKED_RICE.get());
            } else if (servingItem.get() == BuiltInRegistries.ITEM.get(ResourceLocation.fromNamespaceAndPath("farmersdelight", "fried_rice"))) {
                return new ItemStack(UbesDelightItems.LEAF_FEAST_FRIED_RICE.get());
            } else if (servingItem.get() == UbesDelightItems.SINANGAG.get()) {
                return new ItemStack(UbesDelightItems.LEAF_FEAST_SINANGAG.get());
            }
        }

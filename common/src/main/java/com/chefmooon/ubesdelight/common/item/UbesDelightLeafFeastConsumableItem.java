...
public class UbesDelightLeafFeastConsumableItem extends UbesDelightConsumableItem {
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (blockState.getBlock() instanceof BaseLeafFeastBlock baseLeafFeastBlock) {
            if (baseLeafFeastBlock.use(blockState, context.getLevel(), blockPos, player, player.getUsedItemHand(), new BlockHitResult(context.getClickLocation(), context.getClickedFace(), context.getClickedPos(), context.isInside())).consumesAction()) {
